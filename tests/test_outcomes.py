...
@pytest.fixture
def mock_db() -> Generator[Tuple[AsyncMock, AsyncMock, AsyncMock], None, None]:
...
def test_create_outcome(mock_db: Tuple[AsyncMock, AsyncMock, AsyncMock]) -> None:
    mock_create, _, _ = mock_db
    mock_create.return_value = Outcome(
        id=1, 
        learner_id=1, 
        item_id=1, 
        status="passed", 
...
def test_get_outcomes(mock_db: Tuple[AsyncMock, AsyncMock, AsyncMock]) -> None:
    _, mock_read_all, _ = mock_db
    mock_read_all.return_value = [Outcome(
        id=1, 
        learner_id=1, 
        item_id=1, 
        status="passed", 
...
def test_get_outcome_by_id(mock_db: Tuple[AsyncMock, AsyncMock, AsyncMock]) -> None:
    _, _, mock_read_one = mock_db
    mock_read_one.return_value = Outcome(
        id=1, 
        learner_id=1, 
        item_id=1, 
        status="passed", 
...
def test_get_nonexistent_outcome(mock_db: Tuple[AsyncMock, AsyncMock, AsyncMock]) -> None:
    _, _, mock_read_one = mock_db
